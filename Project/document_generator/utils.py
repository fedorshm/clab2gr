...
def insert_footnote(paragraph, text, base_font_size, footnotes, footnote_per_page=5):
    """
    Вставляет текст в параграф с возможным добавлением сноски.
    Сноска никогда не будет вставляться перед первым словом.
    """
    words = text.split()
    if len(words) < 3:
    ...
    if random.random() < 0.2 and len(footnotes) < footnote_per_page:
        ...
        # Если сноска не добавлена, вставляем текст без изменений
